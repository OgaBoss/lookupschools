                        ...
                        <tbody>
                            @foreach($users as $user)
                                @if($user->hasAccess('user'))
                                    <tr>
                                        <td><a class="user_name" href="#" data-user-id="{{$user->id}}">{{ $user->first_name." ".$user->last_name }}</a> </td>
                                        <td>{{ $user->email}}</td>
                ...
                <ul class="pagination">
                    @if($page_number > 1)
                        @for($i=1;$i<=$page_number;$i++)
                            @if($page == $i)
                                <li class="active"><a href="http://{{$_SERVER['HTTP_HOST']}}/admin/users?page={{ $i }}">{{ $i }}</a></li>
                            @else
